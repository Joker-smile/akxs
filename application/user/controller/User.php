...
class User extends UserBase
{
    ...
    public function login($username = null, $password = null){
        ...
        if($this->request->isPost()){
            ...
            if(is_login()){
                ...
                return $this->fetch($this->user_tplpath.'login.html');
            }
        }
    }
        ...
        if($this->request->isPost()){
            ...
            switch ($step) {
                case 1:
                    ...
                    if(!$uid=model('user')->check_require('username',$data['username'])){
                        $this->error('无此用户！','');
                    }else{
                        $email=model('user')->get_info($uid,'email');
                        Session::set('forget_pwd',['uid'=>$uid,'email'=>$email]);
    ...
    public function check_code($value){
        if(Config::get('web.user_reg_verify')!=1 || Config::get('web.user_login_verify')!=1){
            return true;
        }
